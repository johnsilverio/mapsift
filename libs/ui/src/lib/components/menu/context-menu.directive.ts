  ...
  hostDirectives: [
    {
      ...
      inputs: ['cdkContextMenuTriggerFor: uiContextMenuTriggerFor'],
    },
  ],
})
export class UiContextMenuDirective {
  protected readonly cdkTrigger = inject(CdkContextMenuTrigger, { host: true });
  private readonly destroyRef = inject(DestroyRef);
  private readonly document = inject(DOCUMENT);
  private readonly elementRef = inject(ElementRef);

  readonly uiContextMenuTriggerFor = input.required<TemplateRef<void>>();
  noopFn = noopFn;
