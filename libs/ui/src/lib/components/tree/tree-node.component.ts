...
@Component({
  ...
  imports: [NgTemplateOutlet, FormsModule, UiIconComponent, UiCheckboxComponent],
  template: `
    <div
    ...
    >
      ...
      @if (checkable()) {
        <span class="mr-0.5 ml-1.5">
          <ui-checkbox
            ...
            [uiDisabled]="node().disabled ?? false"
      ...
      >
        @if (nodeTemplate(); as tmpl) {
          ...
          @if (node().icon) {
            <ui-icon [uiType]="$any(node().icon)" class="size-4 shrink-0" />
          }
