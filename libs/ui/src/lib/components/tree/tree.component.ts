...
@Component({
  selector: 'ui-tree',
  imports: [UiTreeNodeComponent, CdkVirtualScrollViewport, CdkFixedSizeVirtualScroll, CdkVirtualForOf],
  template: `
    @if (uiVirtualScroll()) {
      <cdk-virtual-scroll-viewport [itemSize]="uiVirtualItemSize()" class="h-full w-full">
        <ui-tree-node
          *cdkVirtualFor="let flatNode of flattenedNodes(); trackBy: trackByKey"
          ...
          [attr.aria-selected]="uiSelectable() ? treeService.isSelected(flatNode.node.key) : null"
          [attr.aria-disabled]="flatNode.node.disabled || null"
          [attr.data-key]="flatNode.node.key"
        />
      </cdk-virtual-scroll-viewport>
    } @else {
      @for (node of uiData(); track node.key; let i = $index) {
        <ui-tree-node
          [node]="node"
          [level]="0"
          [selectable]="uiSelectable()"
          ...
          [attr.aria-level]="1"
          [attr.aria-setsize]="uiData().length"
          [attr.aria-posinset]="i + 1"
          [attr.aria-selected]="uiSelectable() ? treeService.isSelected(node.key) : null"
...
export class UiTreeComponent<T = any> {
  ...
  readonly uiData = input<TreeNode<T>[]>([]);
  readonly uiSelectable = input(false, { transform: booleanAttribute });
  readonly uiCheckable = input(false, { transform: booleanAttribute });
  readonly uiExpandAll = input(false, { transform: booleanAttribute });
  readonly uiVirtualScroll = input(false, { transform: booleanAttribute });
  readonly uiVirtualItemSize = input(32, { transform: numberAttribute });

  readonly uiNodeClick = output<TreeNode<T>>();
  readonly uiNodeExpand = output<TreeNode<T>>();
  readonly uiNodeCollapse = output<TreeNode<T>>();
  readonly uiSelectionChange = output<TreeNode<T>[]>();
  readonly uiCheckChange = output<TreeNode<T>[]>();
  ...
  constructor() {
    ...
    effect(() => {
      this.treeService.setData(this.uiData());
    });
    ...
    effect(() => {
      ...
      if (clicked) {
        this.uiNodeClick.emit(clicked.node);
      }
    });
    ...
    effect(() => {
      ...
      if (keys.size > 0) {
        this.uiSelectionChange.emit(this.treeService.getSelectedNodes());
      }
    });
    ...
    effect(() => {
      ...
      if (keys.size > 0) {
        this.uiCheckChange.emit(this.treeService.getCheckedNodes());
      }
    });
  }
  ...
  private expandFocusedNode() {
    ...
    if (current?.expandable && !this.treeService.isExpanded(current.node.key)) {
      ...
      this.uiNodeExpand.emit(current.node);
    }
  }
  ...
  private collapseFocusedNode() {
    ...
    if (current && this.treeService.isExpanded(current.node.key)) {
      ...
      this.uiNodeCollapse.emit(current.node);
    }
  }
  ...
  private checkFocusedNode() {
    const current = this.getFocusedNode();
    if (current && !current.node.disabled && this.uiCheckable()) {
