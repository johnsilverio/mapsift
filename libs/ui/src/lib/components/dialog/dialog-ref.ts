...
import type { UiDialogComponent, UiDialogOptions } from './dialog.component';
...
export class UiDialogRef<T = unknown, R = unknown, U = unknown> {
  ...
  constructor(
    ...
    private config: UiDialogOptions<T, U>,
    private containerInstance: UiDialogComponent<T, U>,
    ...
    if ((this.config.uiMaskClosable ?? true) && isPlatformBrowser(this.platformId)) {
      this.overlayRef
  ...
  private trigger(action: eTriggerAction) {
    const trigger = { ok: this.config.uiOnOk, cancel: this.config.uiOnCancel }[action];
