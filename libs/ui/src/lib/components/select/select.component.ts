...
import { UiSelectItemComponent } from './select-item.component';
import {
...
@Component({
  ...
  imports: [OverlayModule, UiBadgeComponent, UiIconComponent],
  template: `
    <button
      type="button"
      role="combobox"
      ...
      [attr.data-placeholder]="!uiValue() ? '' : null"
    ...
    >
      <span class="flex min-w-0 flex-1 flex-wrap items-center gap-2">
        @for (label of selectedLabels(); track label) {
          ...
          <span class="text-muted-foreground truncate">{{ uiPlaceholder() }}</span>
        }
      </span>
      <ui-icon uiType="chevron-down" uiSize="lg" class="opacity-50" />
    </button>
...
export class UiSelectComponent implements ControlValueAccessor, AfterContentInit, OnDestroy {
  ...
  readonly selectItems = contentChildren(UiSelectItemComponent);
  ...
  readonly uiLabel = input<string>('');
  readonly uiMaxLabelCount = input<number>(1);
  readonly uiMultiple = input<boolean>(false);
  readonly uiPlaceholder = input<string>('Select an option...');
  readonly uiSize = input<UiSelectSizeVariants>('default');
  readonly uiValue = model<string | string[]>(this.uiMultiple() ? [] : '');

  readonly uiSelectionChange = output<string | string[]>();
  ...
  readonly selectedLabels = computed<string[]>(() => {
    const selectedValue = this.uiValue();
    if (this.uiMultiple() && Array.isArray(selectedValue)) {
  ...
  selectItem(value: string, label: string) {
    ...
    this.uiValue.update(selectedValues => {
    ...
    this.onChange(value);
    this.uiSelectionChange.emit(this.uiValue());
  ...
  private navigateTo(element: UiSelectItemComponent, index: number): void {
  ...
  private provideLabelsForMultiselectMode(selectedValue: string[]): string[] {
    const labelsToShowCount = selectedValue.length - this.uiMaxLabelCount();
    const labels = [];
    let index = 0;
    for (const value of selectedValue) {
      ...
      if (labelsToShowCount && this.uiMaxLabelCount() && index === this.uiMaxLabelCount()) {
  ...
  private provideLabelForSingleSelectMode(selectedValue: string): string[] {
    const manualLabel = this.uiLabel();
    if (manualLabel) {
      return [manualLabel];
    }
  ...
  private getMatchingItem(value: string): UiSelectItemComponent | undefined {
    return this.selectItems()?.find(item => item.uiValue() === value);
  }
      ...
      afterNextRender(() => {
        ...
        const textElements = Array.from(
          overlayPaneElement.querySelectorAll<HTMLElement>(
            'ui-select-item > span.truncate, [ui-select-item] > span.truncate',
          ),
        );
  ...
  private getSelectItems(ignoreFilter = false): HTMLElement[] {
    ...
    return Array.from(dropdownElement.querySelectorAll<HTMLElement>('ui-select-item, [ui-select-item]')).filter(
  ...
  private focusSelectedItem() {
    ...
    let selectedValue;
    if (Array.isArray(this.uiValue()) && this.uiValue().length) {
      [selectedValue] = this.uiValue();
    } else {
      selectedValue = this.uiValue();
    }
  ...
  writeValue(value: string | string[] | null): void {
    if (this.uiMultiple() && Array.isArray(value)) {
      this.uiValue.set(value);
    } else {
      this.uiValue.set(value ?? '');
    }
  }
