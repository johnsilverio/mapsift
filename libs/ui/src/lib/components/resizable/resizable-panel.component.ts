import {
  booleanAttribute,
  ChangeDetectionStrategy,
  Component,
  computed,
  ElementRef,
  inject,
  input,
  ViewEncapsulation,
} from '@angular/core';

import type { ClassValue } from 'clsx';

import { resizablePanelVariants } from './resizable.variants';
import { mergeClasses } from '../../utils/merge-classes';

@Component({
  selector: 'ui-resizable-panel',
  standalone: true,
  template: `
    <ng-content />
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  host: {
    '[class]': 'classes()',
    '[attr.data-collapsed]': 'isCollapsed()',
  },
  exportAs: 'uiResizablePanel',
})
export class UiResizablePanelComponent {
  readonly elementRef = inject(ElementRef);

  readonly uiDefaultSize = input<number | string | undefined>(undefined);
  readonly uiMin = input<number | string>(0);
  readonly uiMax = input<number | string>(100);
  readonly uiCollapsible = input(false, { transform: booleanAttribute });
  readonly uiResizable = input(true, { transform: booleanAttribute });
  readonly class = input<ClassValue>('');

  protected readonly isCollapsed = computed(() => {
    const element = this.elementRef.nativeElement as HTMLElement;
    const width = Number.parseFloat(element.style.width || '0');
    const height = Number.parseFloat(element.style.height || '0');
    return width === 0 || height === 0;
  });

  protected readonly classes = computed(() =>
    mergeClasses(resizablePanelVariants({ uiCollapsed: this.isCollapsed() }), this.class()),
  );
}
