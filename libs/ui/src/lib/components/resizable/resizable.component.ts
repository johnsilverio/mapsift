...
import { UiResizablePanelComponent } from './resizable-panel.component';
import { resizableVariants, type UiResizableLayoutVariants } from './resizable.variants';
import { mergeClasses } from '../../utils/merge-classes';

export interface UiResizeEvent {
  sizes: number[];
  layout: 'horizontal' | 'vertical';
}

@Component({
  selector: 'ui-resizable, [ui-resizable]',
  template: `
    <ng-content />
  `,
  ...
  host: {
    '[class]': 'classes()',
    '[attr.data-layout]': 'uiLayout()',
  },
  exportAs: 'uiResizable',
})
export class UiResizableComponent implements AfterContentInit, OnDestroy {
  ...
  readonly uiLayout = input<UiResizableLayoutVariants>('horizontal');
  readonly uiLazy = input(false, { transform: booleanAttribute });
  readonly class = input<ClassValue>('');

  readonly uiResizeStart = output<UiResizeEvent>();
  readonly uiResize = output<UiResizeEvent>();
  readonly uiResizeEnd = output<UiResizeEvent>();

  readonly panels = contentChildren(UiResizablePanelComponent);
  readonly panelSizes = signal<number[]>([]);
  protected readonly isResizing = signal(false);
  protected readonly activeHandleIndex = signal<number | null>(null);
  protected readonly classes = computed(() =>
    mergeClasses(resizableVariants({ uiLayout: this.uiLayout() }), this.class()),
  );
  ...
  private initializePanelSizes(): void {
    ...
    const sizes = panels.map(panel => {
      const defaultSize = panel.uiDefaultSize();
  ...
  startResize(handleIndex: number, event: MouseEvent | TouchEvent): void {
    ...
    this.uiResizeStart.emit({ sizes, layout: this.uiLayout() ?? 'horizontal' });
  ...
  private handleResize(
    ...
    const leftMin = this.convertToPercentage(leftPanel.uiMin(), containerSize);
    const leftMax = this.convertToPercentage(leftPanel.uiMax(), containerSize);
    const rightMin = this.convertToPercentage(rightPanel.uiMin(), containerSize);
    const rightMax = this.convertToPercentage(rightPanel.uiMax(), containerSize);
    ...
    if (Math.abs(totalSize - originalTotal) < 0.01) {
      ...
      this.uiResize.emit({ sizes: newSizes, layout: this.uiLayout() ?? 'horizontal' });
    }
  }
  ...
  private endResize(): void {
    ...
    const sizes = [...this.panelSizes()];
    this.uiResizeEnd.emit({ sizes, layout: this.uiLayout() ?? 'horizontal' });
  }
  ...
  collapsePanel(index: number): void {
    ...
    if (isCollapsed) {
      const containerSize = this.getContainerSize();
      const defaultSize = this.convertToPercentage(panel.uiDefaultSize() ?? 100 / panels.length, containerSize);
    ...
    this.panelSizes.set(sizes);
    this.updatePanelStyles();
    this.uiResize.emit({ sizes, layout: this.uiLayout() ?? 'horizontal' });
  }
