  ...
  template: `
    <div [class]="classes()" role="group" [attr.data-orientation]="'horizontal'">
      @for (item of items(); track item.value; let i = $index) {
        ...
        >
          @if (item.icon) {
            <span ui-icon [uiType]="item.icon" class="h-4 w-4 shrink-0"></span>
          }
          @if (item.label) {
...
export class UiToggleGroupComponent implements ControlValueAccessor {
  readonly uiMode = input<'single' | 'multiple'>('multiple');
  readonly uiType = input<'default' | 'outline'>('default');
  readonly uiSize = input<'sm' | 'md' | 'lg'>('md');
  readonly value = input<string | string[]>();
  readonly defaultValue = input<string | string[]>();
  readonly disabled = input<boolean>(false);
  readonly class = input<ClassValue>('');
  readonly items = input<UiToggleGroupItem[]>([]);
  ...
  toggleItem(item: UiToggleGroupItem) {
