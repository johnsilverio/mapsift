...
Mapsift runs Vitest through the `@angular/build:unit-test` builder, never the Vitest CLI directly. The builder discovers `*.spec.ts` files and wires the `tsconfig` path aliases (e.g. `@mapsift/ui` → `libs/ui`) that a direct `vitest` invocation fails to resolve.

### Installation

```bash
npm install -D @testing-library/angular @testing-library/jest-dom @testing-library/user-event
```

### Configuration

The `test` target in the project's `angular.json` (or `project.json`) uses the unit-test builder:
...
Path aliases come from the workspace `tsconfig`, so `@mapsift/ui` resolves the same way in tests as in the app:
...
// tsconfig.json (workspace)
{
  "compilerOptions": {
    "paths": {
      "@mapsift/ui": ["libs/ui/src/public-api.ts"]
    }
  }
}
```
...
```bash
# Run tests (through the builder)
ng test

# Single run, no watch
ng test --watch=false

# Specific project in the monorepo
ng test web
```
