...
**Status.** `resource()`, `rxResource()` and `httpResource()` are **stable since v22.0**. Prefer them for
data loading; `HttpClient` stays for imperative flows that genuinely need RxJS operators.

**In Mapsift.** Never hardcode an API URL, read it from the environment config. Interceptors are functional
only. Request and response types come from the **generated** OpenAPI types, never hand-written duplicates
that can drift from the contract (C5, PRD M12). And remember what does not travel this path: element edits go
through the operation queue in the shared core, not through an ad hoc HTTP call.
