...
description: Implement signal-based reactive state management in Angular v22. Use for creating reactive state with signal(), derived state with computed(), dependent state with linkedSignal(), and side effects with effect(). Triggers on state management questions, converting from BehaviorSubject/Observable patterns to signals, or implementing reactive data flows.
---

# Angular Signals

Signals are Angular's reactive primitive for state management. They provide synchronous, fine-grained
reactivity.

**Zoneless is the default since v21**, so signals are how a change reaches the view: there is no Zone patching
async APIs to trigger change detection. `resource()`, `rxResource()` and `httpResource()` are stable since
v22.0 (see the `angular-http` skill).

**In Mapsift**, signals hold **UI state**. Client domain state (the operation queue, optimistic apply,
conflict detection, geometry) lives in the shared Rust core behind a serializable boundary, not in a service
full of signals (C11). Update collections by replacing the reference, never by mutating in place.
