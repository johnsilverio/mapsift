...
Create standalone components for Angular v22. Components are standalone by default, so do NOT set
`standalone: true`.

**Generate, then edit.** `ng g c features/<feature>/<name>` emits the correct shape: standalone,
`ChangeDetectionStrategy.OnPush` (the schematic default since v22), no type suffix, and a separate template
and stylesheet. Never hand-write a component file, and never pass `--inline-template` or `--inline-style`
outside the narrow shared-primitive exception in `.claude/rules/angular.md`. The examples below use inline
templates for brevity only.

**Change detection.** Keep `OnPush`. `ChangeDetectionStrategy.Default` is deprecated in v22 in favour of
`Eager` and is due to be removed; neither belongs in this codebase. The app is zoneless (the default since
v21), so state changes reach the view through signals, not through Zone.
