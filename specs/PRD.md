...
- **Acceptance:** an operation carries its type and operation-schema version, and the four envelope-borne version axes (M10) are present and distinct; the server half is added without altering the client half, and an operation whose claimed author diverges from its session material keeps both the claim and the normalized identity (T5.1); the envelope has exactly one definition in the repository and every language reads a generated form of it.
