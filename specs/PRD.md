...
- **Acceptance:** interrupt a flush after the server applies part of the queue, resend the full queue, and the final state is identical with no duplicated feature and no lost edit; the client advances its cursor from the echoed last-applied; and two clients of the same user (distinct clientIDs) do not collide and neither loses an operation to false dedup.
- **Open / ADR:** the per-clientID cursor needs an expiry and garbage-collection policy (a PRD/ADR item).
...
- **Requirement:** the tenant is the top container of the account (a personal user account or an organization), carried as a tenant identifier on every row and enforced at the SQL layer (row-level security or per-tenant views), so cross-tenant read or write is impossible by construction, including for direct-to-PostGIS readers such as the tile server; the workspace and project below the tenant are organization and permission, not isolation.
- **Basis:** I4, foundation section 9; C4.
- **Provenance:** reopened and closed v0.11 (tenant as the top of the account tree).
- **Acceptance:** a cross-tenant read or write, including a tile request, is impossible by construction and is indistinguishable from a resource that does not exist; the tile role connects non-privileged and sets the tenant on its session, never with RLS bypassed.
- **Open / ADR:** the RLS-versus-views choice and the tile-role session-tenant wiring are a tile and data ADR; the BYPASSRLS hazard (a role owning the tables without FORCE ROW LEVEL SECURITY, or a BYPASSRLS role, defeats the wall) is the load-bearing caveat.
...
- **Acceptance:** every tenant-owned row carries a tenant identifier and no query path, including the ORM, a capability, and the tile role, reads a row whose tenant identifier differs from the session's; creating a personal account creates its tenant with one owner membership and needs no organization; a user belonging to two tenants has one identity and two memberships; a workspace and a project each resolve to exactly one tenant, and an ordinary update cannot move either across tenants.
...
- **Open / ADR:** the concrete identifier variant is an ADR that weighs index locality in PostgreSQL (a time-ordered identifier) against embedding a device clock the model explicitly distrusts (I10) inside a legal-weight object's identifier. Either choice satisfies the requirement above, and if a time-ordered variant is chosen, the embedded time is never read as evidence of when anything happened; created-at and applied-at (T5.3) remain the only times in the trail.

### M4. The client instance and its server cursor

- **Requirement:** a **clientID** identifies a persistent installation. It is generated by the M3 mechanism, persisted in the local store, stable across restarts, distinct from the user and from the session, and never used as an author identity (authorship is T5.1). The server keeps one cursor per clientID holding the last-applied mutation number. Cursor retention is at least the maximum supported offline window, so a client that was offline for the longest supported period still meets its cursor on return. A flush arriving from a clientID the server holds no cursor for is treated as a new client only when it starts at the first mutation number; a flush that starts above it with no cursor is a typed reconciliation response, never applied optimistically, because applying it blindly would risk re-applying operations the server already holds.
- **Shape:** clientID (identifier, persisted locally); server cursor (clientID, last-applied mutation number, last-seen time).
...
- **Provenance:** the chain closed in foundation v0.8; the append-only-and-reproducible-projection rule is settled this PRD round.
- **Acceptance:** replaying a legal-weight feature's attributed chain in server order reproduces its current authoritative geometry exactly; no code path updates a log entry in place, asserted by a test; deleting a user leaves their operations on a legal-weight feature inspectable with the minimum identification retained; an agent-originated operation is distinguishable in the log from a direct human write by the same user.
...
- **Basis:** foundation section 4 and foundation section 10; I2, I9, I10; T2, T5; N5.
- **Provenance:** settled this PRD round.
- **Acceptance:** given an operation identifier from a user report, the flush decision path is reconstructible end to end; no log line carries coordinates or personal data; every user-visible refusal has a matching record and the reverse; a failure with no user-visible signal and no record fails review.
...
- **Acceptance:** a queue far larger than one practical request flushes to completion in batches with visible progress and no loss; interrupting mid-flush and reconnecting resumes from the echoed last-applied with no duplicate and no gap; batching never breaks mutation-number contiguity.
