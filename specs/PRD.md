...
- **Acceptance:** replaying a legal-weight feature's attributed chain in server order reproduces its current authoritative geometry exactly; **the runtime role holds `SELECT, INSERT` and no `UPDATE, DELETE` on the log, so an in-place rewrite is refused by the database for every caller rather than avoided by the code paths somebody covered (ADR-0005 section 2, addition of 2026-08-07), and the test asserts that refusal while distinguishing it from a statement that matched no rows**; deleting a user leaves their operations on a legal-weight feature inspectable with the minimum identification retained; an agent-originated operation is distinguishable in the log from a direct human write by the same user.
