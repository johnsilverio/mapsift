...
- **Requirement:** every operation carries a per-client monotonic mutation number; the server tracks the per-client last-decided number and ignores any operation at or below it, so a resent flush is idempotent; the server echoes the last-decided number in the flush response and the client advances its cursor only from that echo; a client is a persistent installation (a clientID, generated as in I3), not the user, so the same user on two devices is two clients with non-colliding streams.
- **Basis:** I9, foundation section 4; C12.
- **Provenance:** closed v0.7, echo and clientID-per-installation closed v0.8; **the cursor's meaning and its name revised 2026-09-17 (foundation v0.19, ADR-0014)**, from last-applied to last-decided, because a refusal the cursor does not pass stalls an append-only stream permanently (I2).
- **Acceptance:** interrupt a flush after the server applies part of the queue, resend the full queue, and the final state is identical with no duplicated feature and no lost edit; the client advances its cursor from the echoed last-decided; and two clients of the same user (distinct clientIDs) do not collide and neither loses an operation to false dedup. **Added 2026-08-11 at the MAP-12 pickup:** an operation the server already holds, resent and surviving the dedup filter because it arrived under a different mutation number, is **answered as applied rather than refused**, so the flush succeeds and echoes. C12 exists to make the retry path safe, and a refusal there turns a legitimate resend into an error a client cannot tell apart from a real one, which is the fragility this requirement is against. **Sharpened 2026-09-24 at the MAP-68 pickup (MAP-74), because that sentence predates a log holding refused entries (ADR-0014) and was implemented as a second judgement:** such an operation is answered with **the verdict the log already holds for it**, applied or refused, the cursor passes it, and it **leaves the current state as it found it**, being neither judged, projected nor appended again (an operation is the one its identifier names, M3, whatever a resend carries; the appended half and this parenthesis were added the same day at the pre-dispatch read, as the orchestrator's sharpening reported to the owner). Answering a held refusal as refused is not the refusal this addition forbids, which was refusing a legitimate resend; it reports a decision already taken. Measured through the route the same day: the log keeps the entry it first appended while the projection took the resend's content, so an older geometry replaced a newer one, a feature was re-filed under the layer the resend named, and an operation the log records as refused was projected. Foundation section 4's "nothing is applied twice" is what this serves.
...
- **Acceptance:** an operation by an author who lost write permission while offline is flagged at flush, not applied and not dropped, **and the operations after it in the same stream still reach the server** (added 2026-09-17: the flag is a verdict on that operation alone, ADR-0014, because a whole-batch refusal of an append-only stream is the permanent divergence I2 forbids).
...
- **Provenance:** principle closed v0.11 (foundation section 9); the listing-derives-from-isolation refinement settled this PRD round; the mechanism is the PRD's. **One exception is accepted and recorded, 2026-09-24:** a tenant whose create named a feature identifier another tenant holds is answered, on later operations on that identifier, differently from a tenant without the collision. It is reachable only by minting another tenant's 128-bit identifier, its reasoning is in M9's Provenance, and MAP-75 closes it.
...
- **Provenance:** the granularity ladder closed in foundation v0.2; the single-target addressing rule, the whole-geometry payload, and the named multi-target exceptions were settled in v0.9 as the mechanism that carries them. **The typed, flag-and-retain refusal of a geometry outside its layer's declared family is settled in v0.15**, with M2, and it is the half of that rule which decides whether it protects field work or destroys it. **The wire refusal of a type-to-target-kind mismatch is settled 2026-08-06**, closing the MAP-8 Window A review, because the metadata-only reading left the granularity enforceable by convention alone and the review showed a green suite shipping the defect. **The refusal of an operation that would re-file a feature is settled 2026-09-24**, at the MAP-68 pickup: M2 files a feature under exactly one layer and M3 keeps an identifier to one object for its life, and the route, answering `200` to a create or a set naming a held feature elsewhere, was measured moving it across layers of one project and across projects of one tenant, and putting a geometry of one family under a layer declaring another; and a geometry set naming a feature nothing created inserted it, so a feature came into existence from an operation M9 does not give that power. **What the tenant holds is read from its current state inside the I4 wall, and one exception to T6.5's indistinguishability is accepted with it, on the owner's ruling of the same day.** A create naming an identifier another tenant holds is applied and never projected (ADR-0012 decision 3's addition of 2026-09-11), so that tenant's later operations on the identifier are answered as if the feature were absent, where without the collision they are answered as if it were held: the answer depends on a row behind the wall. It is accepted because reaching it requires minting another tenant's 128-bit identifier, which never crosses the wall (ADR-0005 section 4, ADR-0006), and because in that case the tenant's own state already lacks the feature it created; it is closed by reading what a tenant holds from its own log rather than from the projection, which is MAP-75. The names, the order that picks one reason and the batch's own contribution to what is held are ADR-0010 decision 6's addition of 2026-09-24.
- **Acceptance:** no operation in the catalog targets more than one property of one feature, except the named exceptions, and a test enumerates the catalog and asserts it; two operations on different properties of one feature resolve with no conflict, driven by the target path alone; a geometry conflict presents two whole geometries and produces no third geometry (T3.3); an unknown operation type is rejected with a typed error rather than ignored; a catalog operation addressed at a target kind other than the one its type declares is refused with a typed error, never read at a coarser or finer granularity than the catalog fixes (settled 2026-08-06, closing the MAP-8 test review); and a geometry operation carrying a geometry outside its layer's declared family is refused with a typed error and the operation is **flagged and retained for inspection**, never discarded, the refusal falling on that operation alone so the stream behind it still reaches the server (added 2026-09-17, ADR-0014, after a whole-batch reading of this clause was found to stall an append-only queue permanently against I2), on the same shape T5.2 fixes for the operation whose author lost authorization, because that geometry was drawn offline in the field and a refusal that drops it is the preserve-not-discard sin wearing a validation costume (C7, C13); and a `feature.create` naming a feature the tenant already holds, under whatever layer or project, and any other operation naming a feature the tenant does not hold at the layer and project it names, whether no applied operation ever created it or it is filed elsewhere, are each refused with a typed error on that operation alone and retained the same way, and a held feature stays where it was (added 2026-09-24 at the MAP-68 pickup, the owner's ruling; the reason and the one accepted exception are in Provenance).

### M10. The five version axes and the ordering rule

- **Requirement:** **five** version axes exist, they never substitute for one another, and each has one owner that may increment it. The fifth, the **resync cursor**, is the **per-project version**: server-assigned, monotonic per project, allocated inside the flush transaction under the project lock, and presented by a client to ask for everything that changed since it last looked. It is distinct from the per-feature version, which orders and detects conflict on one feature, and it is what makes version order equal commit order by construction, which is the whole reason the sequence trap does not reach it (ADR-0004). The **per-feature version** is server-assigned and monotonic per feature, and it orders operations and detects conflict. The **per-client mutation number** is client-assigned, monotonic and contiguous **per clientID within one flush domain, which is one tenant and one project** (settled 2026-08-11 at the MAP-12 pickup, replacing a bare "contiguous per clientID"), and it gives idempotency and dedup. The **operation-schema version** is build-time and per operation type, and it drives upcasting (T9.3). The **conflict-rule version** is build-time and per runtime, and it detects temporal skew (T4.3). On top of them one ordering rule closes the last silent-loss hole in the flush path: the server applies a client's operations **only in contiguous mutation-number order**, so an operation at or below the cursor is deduplicated and ignored, and a **gap above the cursor is a typed error that asks the client to resend from the cursor, never a silent skip**, because the client's queue is persistent and append-only and a gap means an operation was lost rather than never created. **"From the cursor" is the cursor plus one, the first mutation number the server has not applied, sharpened 2026-08-13 at MAP-13's Window A review** and fixed with its reasoning in ADR-0010 decision 6's addition of that date. **The sharpening is here rather than only there because this sentence is the higher authority and it read the other way**, so a client built from it would have resent one number below what the server advertises, which is the off-by-one on two sides of a boundary that the decision itself refuses. Found by the Canon axis; the correction had reached this requirement's Acceptance, M4's Acceptance, the ADR and the log, and had not reached the sentence the ADR names as divergent. **That reason is what fixes the domain, and it was read against a second destination for the first time on 2026-08-11.** A gap means loss only while the stream has **one** destination. A flush addresses exactly one tenant and exactly one project (ADR-0010 decision 6 with its addition of 2026-08-10), so a single stream spanning destinations presents the server with holes it **cannot tell apart from loss**: the operation may have gone to another project or another tenant, and no evidence available on this side distinguishes the two. The client therefore mints one contiguous stream **per clientID, tenant and project**, the server's cursor is keyed the same way (M4), and the guarantee is exact rather than best effort. **The alternative was refused with its reason:** widening the flush to a whole tenant would let one batch span projects, and a batch spanning projects cannot allocate its range in one statement, which is ADR-0004's RANGE rule and the measurement behind it.
- **Shape:** per-project version (integer, server-owned, the resync cursor, carried on the envelope's server half per M8 as settled 2026-08-07); per-feature version (integer, server-owned); mutation number (integer, client-owned, contiguous); operation-schema version and conflict-rule version (build-time identifiers carried in the envelope). Every axis is a **distinct type rather than a distinct field name**, so the substitution the acceptance forbids is refused by the compiler and the type checker rather than by a reviewer's attention (settled 2026-08-07 at the MAP-9 pickup, on the MAP-8 precedent that refused a convention-only reading of the same kind of clause). Two runtimes carry that structurally and the third does not: `specs/dependencies.md` section 2 holds the measurement and names TypeScript as the recorded gap. **The mutation number's first value is zero, settled 2026-08-11 at the MAP-12 pickup**, which is what the generated envelope already permits (an unsigned integer with no floor above zero), so nothing in `libs/core` changes in order to say it and the freshness gate is not disturbed. What it costs is that zero cannot serve as a sentinel, which is why M4 represents an absent cursor by the absence of its row. **The gap clause of the Acceptance below was narrowed 2026-08-13 at the MAP-13 pickup, and its argument lives here rather than inside an acceptance line**, which is a list an implementing window scans for what must be true and not a place to bury an essay (the shape of this correction is the Craft axis's). **The wider form had to go** because it permits a flush that both applies and refuses, whose answer would then have to carry a moved cursor and a restart point at once, while C12 has the client advance from one echo and nothing else *(reconciled 2026-09-17: a flush that both applies and refuses is now the ordinary case, and the sentence survives because what it refuses is a **restart point** beside a moved cursor. ADR-0014's answer carries no restart point at all; it carries one echo and a list of verdicts, and the two `409` reasons that do carry a restart point still refuse the batch whole. C12 is untouched)*; the batch's own internal contiguity is a composition rule refused at the boundary in ADR-0010 decision 6 rather than a case of that clause. **The contrast the narrowing draws has no witness, and that is recorded rather than rediscovered later:** once the composition rule exists, a batch reaching the cursor comparison is already internally contiguous, so a batch starting above the cursor plus one carries nothing the server could have applied and the two forms coincide. The batch that would separate them, a cursor at one against a batch of two, three, seven and eight, is refused at the boundary a step earlier. The clause keeps its teeth against an implementation that refuses and appends anyway, and **it must not be read as pinned by a case that tells the two forms apart**, because none can exist while the composition rule stands. Reached independently by the implementing window and by the Spec axis.
...
- **Acceptance:** the five axes are distinct fields and no code path reads one as another; a resent flush is deduplicated by mutation number with no duplicate and no loss (T2.3), the cursor the dedup reads being the last-**decided** rather than the last-applied since 2026-09-17 (foundation v0.19, ADR-0014), so a refused operation is deduplicated on resend exactly as an applied one is and the stream behind it is never stalled; a flush with a gap above the cursor returns a typed resend-from-cursor response and **applies nothing at all, rather than merely nothing from the gap onward** (narrowed 2026-08-13 at the MAP-13 pickup; the argument for the narrowing and the note that its contrast has no witness are in Shape above, and the response's status and shape are ADR-0010 decision 6's addition of the same date); a per-feature version never decreases.
...
- **Shape:** log entry (the M8 envelope as applied, immutable); projection (current features and layers, derived); legal-weight chain (the ordered attributed subset for one feature). **A refused operation is a log entry as well** (added 2026-09-17, ADR-0014): it carries its verdict and its reason, it is what makes T5.2's and M9's retention real, and it is **not** part of the projection or of the replay the reproducibility clause runs.
...
- **Open / ADR:** retention, archival, and whether a legal-weight project can be physically deleted at all are OQ-20; **the projection's materialization strategy is CLOSED by ADR-0012 (2026-08-21)**: a maintained current-state table on `layers_feature`, written inside the flush transaction, with replay on read rejected on a measured 96 times in its best correct form and 756 times written naively, on the read the product's own paths perform. That ADR also settles where the applied server half is stored and gives `applied_at` the column this requirement and T5.3 have always required and nothing carried (MAP-53), and it records what it deliberately does not fix: under the isolation policy that read takes no spatial index at all (MAP-51). **That last clause was closed on 2026-08-25 by ADR-0013 and is narrower than written:** a read naming a **container** (a layer, a layer set or a project) as well as the tenant does take an index, built entirely out of `uuid_eq`, and it is only an **unqualified** read that takes none. No PostGIS predicate is marked `LEAKPROOF` by this product, and every sanctioned spatial read names a container. **The reproducibility clause above did not discriminate between the two strategies** and the ADR says so rather than claiming its support: M9 forbids a vertex delta, so replaying a feature's geometry is a latest-row-per-target-path query that both shapes reproduce exactly, and what separates them is drift.
...
- **Open / ADR:** the backend, the sampling policy, the dashboards and the alerting are an ADR whose trigger is the first real users (foundation section 10, v0.16); the client telemetry SDK is part of the same ADR, surveyed in `specs/dependencies.md`. **The logging library is CLOSED, 2026-08-14, by ADR-0011**, which takes the standard library with a JSON formatter, the correlation keys bound once per context rather than passed by callers, and redaction as a closed allowlist on the root handler. **Two things about that closure are worth the sentence.** It was decided at the first code that logs (MAP-14) rather than at the first real users, because the log path is the half of N9 that cannot be retrofitted, which is the same reasoning that put the requirement here at all. And **this line had widened the foundation's deferral**: foundation section 10's deferred list names the backend, the sampling, the dashboards and the alerting, and never named the logging library, so the derived document had deferred something the authority left available. Corrected here rather than by loosening the foundation, since the foundation was the one that was right.
