#!/usr/bin/env bash
# Fixed 440,000 rows per tenant, two tenants. P projects per tenant; each project holds a
# 1,000-row layer (the one read) and a filler layer. Only P varies, so the skipped column's
# cardinality is the single independent variable.
set -euo pipefail
C="$1"
for P in 4 20 100 400; do
  FILL=$(( 440000 / P - 1000 ))
  docker exec -i "$C" psql -q -U probe -d probe -P pager=off <<SQL 2>&1 | grep -E "^\S*\|" || true
...
CREATE INDEX p_i ON ps.feature (tenant_id, project_id, layer_id);
ANALYZE ps.feature;
SQL
  for run in 1 2 3; do
...
SELECT count(*) FROM ps.feature WHERE layer_id='cccccccc-0000-0000-0000-000000000001'::uuid
  AND ST_Intersects(geometry, ST_MakeEnvelope(-47.544,-17.544,-47.456,-17.456,4674));
ROLLBACK;
SQL
)
  done
  META=$(docker exec -i "$C" psql -qAt -U probe -d probe -P pager=off -c \
    "SELECT (SELECT relpages FROM pg_class WHERE oid='ps.p_i'::regclass)||'|'||(SELECT count(*) FROM ps.feature)")
  echo "P=$P|$META|$(printf '%s' "$OUT" | tr -d '\n')"
done
