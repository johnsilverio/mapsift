...
>
> **How to read an entry.** Each carries a **status**, what was **verified and when and against which source**, and the **particularity** that changes how it is used. Two words are not interchangeable here. **Ratified** means the foundation closed the *choice* (Django is the backend). **Pinned** means a *version* is fixed, which only a lockfile can do, and the three that do it are `apps/api/uv.lock`, `libs/core/Cargo.lock` and `package-lock.json` (corrected 2026-08-10: this header still said no lockfile existed, which stopped being true at the scaffold and was contradicted by entries below that cite pinned numbers). **An entry that names a version says which of the two it is**, and where it does not, the lockfile is the answer and this document is the reasoning.
>
...
### The type-generation toolchain: two halves ratified by ADR-0009, the Dart half still open

> **Corrected and decided 2026-08-05 (MAP-31, ADR-0009).** The paragraph this replaces claimed Specta "exports TypeScript (stable), Swift, Go, Kotlin, Python, C#, Java, JSON Schema, Zod, and OpenAPI" and that the TypeScript side "comes from wasm-bindgen". Both failed verification against primary sources: Specta's own README marks only TypeScript and Swift production-ready, `specta-jsonschema` is 0.0.3 with 129 total downloads, `specta-python` is 0.0.1 with roughly five lines of Rust, and the core has sat at 2.0.0-rc since 2023; and wasm-bindgen (0.2.126) emits `any` for every serde-serialized value (its issue #1591), so named TypeScript types do not fall out of it for a data contract.

The ratified toolchain for the envelope contract. **ADR-0009 is the decision; this section is the survey detail**, every claim verified 2026-08-05 against crates.io, PyPI, the repositories and empirical probes:

- **schemars 1.2.2** (released 2026-07-27, active): emits JSON Schema, default dialect 2020-12 but documented as liable to change, so the pin is explicit (`SchemaSettings::draft2020_12()`). Serde-attribute fidelity (rename, rename_all, tag/content, untagged, flatten, skip, default) verified empirically on the envelope's shapes; tagged unions emit `oneOf` with a `const` discriminator; `Uuid` maps to `format: uuid` and `chrono::DateTime<Utc>` to `format: date-time` (features `uuid1`, `chrono04`); output byte-deterministic across runs. Four particularities, the middle two confirmed in the MAP-7 window (2026-08-06). `serde_json::Value` emits the boolean schema `true` only when the field carries no doc comment; with one, schemars wraps it as an annotation-only object schema, and datamodel-code-generator 0.72.1 produces `Any` either way, probed both ways, so the `#[schemars(schema_with = ...)]` escape hatch is unnecessary at these pins. schemars emits no OpenAPI `discriminator` keyword, which datamodel-code-generator needs to route by tag rather than by trial; `#[schemars(extend("discriminator" = {"propertyName": ...}))]` supplies it. A required-but-nullable wire key comes from the emitter's **serialize contract** (`SchemaSettings::draft2020_12().for_serialize()`), never from `#[schemars(required)]`, which looks like the same fix and is not: the attribute also strips the null and inlines the referenced definition, so the reader would refuse the explicit `null` the wire carries (probed 2026-08-06; the emitter's trap comment guards it). Unsigned integers carry their floor and not their ceiling: `u32`/`u64` map to `integer` with `minimum: 0` and no `maximum` (the generator warns `format of 'uint32' not understood` and falls back to `int`), so the Python reader accepts a value above the Rust range; recorded rather than fixed, because every producer of these fields is one of the two constrained runtimes. And key ordering flips once if serde_json's `preserve_order` feature ever unifies on from another workspace crate, which the diff gate then flags. And one serde-side trap the MAP-8 implementation window measured (2026-08-06): `#[serde(tag = ...)]` on a **struct** writes the tag on serialize and ignores it entirely on deserialize (a wrong or a missing tag parses), and schemars emits no tag property for it at all, so an untagged union over such structs routes by first fit; the only derive-only shape that validates the tag and emits a `const` is a single-variant internally-tagged **enum**, and a fieldless enum is no substitute, because it emits `enum`, which datamodel-code-generator turns into a `StrEnum` whose Pydantic error type is `enum` rather than `literal_error`. And one consequence of the same doc-comment mechanism (the MAP-8 polish round, 2026-08-06): a `///` on a serialized field is a **contract change, never documentation alone**, because schemars lifts it into the schema as `description` and datamodel-code-generator renders it as `Field(description=...)`, distributed into every union member carrying the field; both freshness gates catch it, so the cost is a regeneration, never drift. And the third face of that same mechanism, probed at the MAP-9 boundary decision (2026-08-07): a `#[serde(transparent)]` newtype reaches `$defs` as its own `$ref` **only when it carries a doc comment**; without one schemars inlines it, so the field emits as a bare `integer` indistinguishable from the primitive. **The dependency is on the attribute and not on the newtype**, re-probed as a full matrix on 2026-08-07 after the MAP-9 implementation window removed `transparent` and the first reading turned out to have mistaken a correlation for a cause: a **plain** newtype reaches `$defs` with or without a doc comment, and only the `transparent` one inlines when undocumented. So the rule for this repository is the simple one, **declare an axis newtype without `#[serde(transparent)]`**, which is redundant anyway (serde already serializes a newtype struct as its inner value, and removing it from all five left the emitted schema byte-identical). The hazard survives only as a latent pair: adding `transparent` to an undocumented newtype degrades it to a bare integer. **What holds that line is a test and not a gate**, measured by the MAP-9 implementation window (2026-08-07) after this entry first claimed the opposite: `just contracts` fails once on the stale artifact and then goes green on the regeneration its own message tells you to run, because both freshness gates compare a generated form against its source and neither can ask whether that source still means what it did. The degradation surfaces in `apps/api/mapsift/sync/tests/test_generated_envelope.py`, where the axis read through `.root` raises `AttributeError` on a bare integer. **A generated contract's freshness gate is a staleness check, never a semantic one**, and that generalizes past this case.
- **datamodel-code-generator 0.72.1** (released 2026-08-04, near-weekly cadence, the tool the official Pydantic docs point to for code generation): Pydantic v2 output via `--output-model-type pydantic_v2.BaseModel`; discriminated unions generated correctly from `oneOf` plus a discriminator; byte-determinism guarded by an upstream test suite since 0.67.0 plus `--disable-timestamp`; the native `--check` flag is the CI freshness gate; output for the envelope's shape passes `mypy --strict` unmodified (verified with mypy 2.3.0, pydantic 2.13.4, Python 3.13). Particularities: Beta-classified 0.x whose output changes across minors and whose default formatters are becoming opt-in, so the generator, the formatter set and `--target-python-version` are pinned together; the generated header embeds the input filename, so the invocation path is fixed; a duplicated or unresolvable discriminator silently degrades to a plain union, which is why ADR-0009 section 5's guard test exists. And one measured at the MAP-9 boundary decision (2026-08-07): a `$defs` entry that is a bare scalar generates as `RootModel[int]` under these flags, which **is nominally distinct under mypy `--strict`** (measured with mypy 2.3.0 and pydantic 2.13.4: substituting one axis newtype for another raises `arg-type`), and its wire form is unchanged (bare integer in, bare integer out, round trip byte-identical). **That protection has one named hole and it is this repository's own configuration** (found by the MAP-9 Window A, re-measured in the api container 2026-08-07): `[tool.pydantic-mypy] init_typed = false` in `apps/api/pyproject.toml` makes the plugin synthesize an `__init__` taking `Any` per field, so a substitution passed **through a model constructor** is silent, while the same substitution as a function argument (`arg-type`) or as an annotated read (`assignment`) still fires. The setting is deliberate and has its own reason recorded beside it, so the hole is worked around rather than closed: **anchor a distinctness assertion on the read path, never on construction**, which is also the path M10's clause actually names. The cost is on the read side: the parsed attribute is the model and not the scalar, so every server-side read of such a field goes through `.root`.
- **tsify 0.5.6** (released 2025-10-14; the tsify-next fork is officially retired back into the original): `#[derive(Tsify)]` with the `js` feature (serde-wasm-bindgen) emits named interfaces and discriminated unions into the wasm-pack pkg `.d.ts` via `typescript_custom_section`, and the 0.5.x `Ts<T>` wrapper types the crossing signatures. Maintenance is the thin point: two releases in eighteen months, one maintainer, wasm-bindgen pinned only at the `0.2` range. The recorded exit path is **ts-rs 12.0.1** (released 2026-01-31, actively maintained) emitting one file per type into `libs/contracts` under `TS_RS_EXPORT_DIR`, gated by regenerate-and-diff, with hand-annotated `unchecked_return_type` and `unchecked_param_type` overrides as the cost. The MAP-7 window confirmed empirically (2026-08-06): the tagged-union output is a real TypeScript discriminated union (the `never`-branch test compiles against it); an `Option` that must serialize as explicit null needs `#[tsify(missing_as_null)]`, because tsify's default is `undefined`; and tsify maps neither `Uuid` nor `DateTime<Utc>` (it emits the bare Rust type name, an undefined TypeScript type), so those fields carry per-field `#[tsify(type = "string")]`, verified against the crate source as the honest minimum. The MAP-8 window added one more (2026-08-06): a `@ts-expect-error` directive opening a multi-line comment still binds to the next code line on this toolchain (Angular 22, Vitest via the unit-test builder) and counts as used, probed with a genuine TS2322 under the two-line comment form, so the long-form annotated directives in the contract specs are sound. The implementation round added the flatten defect and its remedy (2026-08-06): the parser branches on whether a flattened type is syntactically a type reference, so `#[serde(flatten)]` of a named union emits `interface X extends Union`, invalid TypeScript that `skipLibCheck` swallows while the member keys vanish; `#[tsify(type = "...")]` on the flattened field takes the intersection branch instead (ratified, ADR-0009 section 3 note). The MAP-9 boundary probe added the newtype case (2026-08-07): a `#[serde(transparent)]` newtype deriving `Tsify` emits `export type Name = number` and the consuming field is typed to that alias, with **no dependency on a doc comment** (unlike the schemars side above) and none of the undefined-bare-name defect `Uuid` and `DateTime<Utc>` carry. **Protection is nevertheless nil**, because a TypeScript alias is structural: the alias and the primitive are mutually assignable, so the substitution a newtype forbids in Rust and in Python still compiles here. Recorded rather than closed: a branded type is the only mechanism that would close it and it would be a hand-written declaration of a generated type, which ADR-0009 section 4 refuses, and the ts-rs exit path carries no branding either. ts-rs output determinism stays unconfirmed and is checked only if the exit path fires.
- Disqualified with the reason on record: **Specta** (the corrected claim above); **serde-generate / serde_reflection** (its Python output is plain dataclasses aimed at binary formats, and its runtime tracing struggles with format-dependent impls like `serde_json::Value`, the exact field the envelope carries); **Typeshare** (no Python and no JSON Schema target, and static source analysis that loses information).

**Type generation to Dart remains the open hole it was, and it must not be assumed solved.** The foundation (9.6.2) and PRD M12 require Dart when mobile exists, and no tool above emits it. The leading candidate on 2026-08-05 is **flutter_rust_bridge 2.12.0** (active), and it is a full bridge that owns the FFI layer and the generated Dart API surface, a boundary-architecture commitment rather than a types-only emitter, which is why it waits for `apps/mobile` and its own ADR round (ADR-0009 section 6).
...
| A structured-logging library (`structlog` with `django-structlog`, against the standard library plus a JSON formatter) | **DECIDED 2026-08-14 by ADR-0011: the standard library with a JSON formatter**, and this row is the survey it walked through rather than the decision | The survey leaned the other way and was **reversed with its reasons on the record**, which the ADR carries in full. Not a compatibility refusal: *verified 2026-08-14 against the package index*, `structlog` **26.1.0** (2026-06-06) needs Python 3.10 or later and carries no runtime dependency on 3.13, and `django-structlog` **10.1.0** (2026-05-30) declares Django 4.2 through 5.2 and Python 3.10 through 3.14, so both fit the pinned Django 5.2.16 and Python 3.13.13. **The particularity that decided it, and it is the kind this survey exists to catch:** `django-structlog` requires **`django-ipware`** as a mandatory dependency and binds the client `ip` per request by default, with `user_agent` on its request events, which is precisely the data PRD N9 and N5 keep out of a log; a setting disables it (`DJANGO_STRUCTLOG_IP_LOGGING_ENABLED`), and a library that is safe because a setting is right is the caller diligence N9 replaced with a path. Two supporting facts: the key it binds free is the request identifier, one of N9's four, and its Celery integration has no consumer because Celery is deliberately not installed. |
...
15. **The observability toolchain** (added 2026-08-03, foundation v0.16). **Narrowed 2026-08-14 from four
    choices to three:** the structured-logging library left this item and was decided by **ADR-0011** at
    MAP-14, because the first code that logs arrived before the first user did and the log path is what
    cannot be retrofitted. What remains here is the client telemetry SDK, the backend and its collector, and
    the sampling and alerting policy. **Its trigger is unchanged, the first real users**, not a date and not
    a deployment for its own sake. The survey it walks through is section 4.1 above, and the constraint it
    may not relax is the vendor neutrality the foundation closed, so whatever wins stays swappable.
