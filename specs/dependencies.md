...
- **schemars 1.2.2** (released 2026-07-27, active): emits JSON Schema, default dialect 2020-12 but documented as liable to change, so the pin is explicit (`SchemaSettings::draft2020_12()`). Serde-attribute fidelity (rename, rename_all, tag/content, untagged, flatten, skip, default) verified empirically on the envelope's shapes; tagged unions emit `oneOf` with a `const` discriminator; `Uuid` maps to `format: uuid` and `chrono::DateTime<Utc>` to `format: date-time` (features `uuid1`, `chrono04`); output byte-deterministic across runs. Two particularities: `serde_json::Value` emits the boolean schema `true`, valid 2020-12 but a known interop risk for schema-to-code tools, with `#[schemars(schema_with = ...)]` as the escape hatch; and key ordering flips once if serde_json's `preserve_order` feature ever unifies on from another workspace crate, which the diff gate then flags.
...
- **tsify 0.5.6** (released 2025-10-14; the tsify-next fork is officially retired back into the original): `#[derive(Tsify)]` with the `js` feature (serde-wasm-bindgen) emits named interfaces and discriminated unions into the wasm-pack pkg `.d.ts` via `typescript_custom_section`, and the 0.5.x `Ts<T>` wrapper types the crossing signatures. Maintenance is the thin point: two releases in eighteen months, one maintainer, wasm-bindgen pinned only at the `0.2` range. The recorded exit path is **ts-rs 12.0.1** (released 2026-01-31, actively maintained) emitting one file per type into `libs/contracts` under `TS_RS_EXPORT_DIR`, gated by regenerate-and-diff, with hand-annotated `unchecked_return_type` and `unchecked_param_type` overrides as the cost. Two checks the pinning window confirms empirically, because neither is documented: tsify's exact output for a `tag`/`content` union, and ts-rs output determinism.
