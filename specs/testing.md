...
**That assembly is read adversarially before Window A rather than after it** (added 2026-08-14, ADR-0008 section 9). The reason is a consequence of this very section: a window is told that handed-over evidence is not its to question, so **a defect in the spec rides through by construction** and is caught by a review axis one full window later, when a correction round is the only remedy left. Seven rounds produced seven blocking findings in orchestrator-owned documents, four of them in the spec itself. So one pass reads the spec alone against the requirement it cites, before anything is dispatched, and it is looking for the two things a window cannot: **a criterion that appears here and nowhere upstream**, and **a claim about the requirement that the requirement does not support**, a block announcing itself as a copy of a set being the worked example.
