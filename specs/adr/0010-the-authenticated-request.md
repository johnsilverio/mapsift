...
> **Addition (2026-08-20), at the MAP-39 pickup: the project claim is verified as well, after the binding rather than before it, and its refusal is the tenant's not-found.** The clause above fixes the tenant refusal before the binding so the wall is never asked to police a value it was configured from, and read cold that sentence sends every not-found refusal to the same position. The project's cannot stand there: `Project` is a tenant-owned row behind the wall, so the read answering whether the batch's project exists in the verified tenant only answers once that tenant is bound (ADR-0005 sections 3 and 4). The order on the flush path is therefore the tenant claim before the binding, the binding, then the project claim, and only then the cursor read, the dedup and the append. The position ahead of the cursor read is load-bearing rather than tidy: the cursor is keyed by clientID, tenant and project, so a batch addressing a project its tenant does not hold would otherwise earn `no_cursor_in_this_domain`, a `409` that tells the client to rehandshake against a project it may not address, which is the wrong recovery arriving with the wrong status.
>
> **Inside the wall, a project of another tenant and a project that never existed are one silence by construction**, which is T6.5's cross-tenant indistinguishability delivered by the mechanism rather than defended against it. The refusal is the same shape as the tenant's: not-found, comparative over status and body under the addition of 2026-08-07, and recorded under ADR-0011 section 4, whose 404 reason value stays the window's to choose.
>
> **What this deliberately does not answer:** T6.5's within-tenant half, the explicit denial with a resolution path, which has no per-project grant to stand on; and T6.4's three sharing modes below membership. Both stay with the detailed permission model those requirements defer to. What stands on this path until then is membership plus containment: a member of the verified tenant addresses that tenant's projects, and no one else's exist.
