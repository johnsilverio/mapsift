...
>
> **What this costs, accepted rather than discovered later:** a counter row can be minted for a project identifier that never existed, so the table does not by itself prove a project is real. Nothing in this strategy asked it to. The claim that a batch addresses a project the principal may write to is authorisation's, which is T6.4 and T6.5 through the tenant, and it is unchanged by this.
