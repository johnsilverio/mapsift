...
>
> **The operational form of LATE, added the same day after an implementation satisfied every case and still grew the critical section.** The lock is taken by the allocation and held until commit, so **no per-operation work may run between the allocation and the insert**: serialising a payload, resolving an address, building a model instance. Doing it there is easy to reach by accident, because a generator passed to `bulk_create` is consumed *inside* that window rather than before it, and the resulting flush holds one project's row across N serialisations instead of across one statement. **That is the exact cost the two rules were measured to remove.** The check a window applies to itself is therefore not "is the allocation last" but **"does anything between the allocation and the insert do work per operation beyond handing out the range"**. **Corrected 2026-08-10, hours after the sentence was written, because its first form asked whether anything in there scales with the batch and that condemns the distribution RANGE itself prescribes**: the range can only be handed out after the allocation answers, so the loop that assigns it is the rule's own instruction rather than a violation of it. Measured on the batch size decision 3 fixes, 500 operations: building the entries costs 11.99 ms and stamping the range costs 0.05 ms, a factor of 222, which is the difference between per-operation work and per-operation assignment. **No test in this suite can see it**: the lock-count case counts statements and says nothing about what runs between them, which puts this in the same category as the escape above, enforced by reading rather than by the build.

> **Extended 2026-08-11, at the MAP-12 pickup, because LATE placed the dedup while the dedup was still only a read.** The bullet lists dedup among what runs before the allocation, and that is right for the **read**: comparing a batch against the per-client cursor takes no lock. **What the bullet never contemplated is that the cursor is itself a row somebody has to write**, because when this was decided no such row existed, and a second row lock inside this transaction is a second thing that can be held too long.
>
> **So the two halves separate, and the separation is the decision.** The cursor is **read early**, as the validation the bullet already calls it, with no lock taken. The cursor is **written late**, in one statement of its own beside the allocation, carrying a guard that refuses to move it backwards, so a flush that lost a race cannot lower a cursor another flush already raised.
>
> **The shape that must not be taken, named for the same reason the `max(project_version)` escape above is named.** Taking the cursor early with `SELECT ... FOR UPDATE`, to make the read-then-write atomic, is the obvious move and it holds that row across the whole flush: the exact cost the two rules were measured to remove, reintroduced on a different table. **The atomicity it reaches for is already bought elsewhere.** The log's `UNIQUE (tenant_id, operation_id)` means a duplicate operation fails its insert whole, so **the cursor is the cheap path and the constraint is the correctness backstop**, never the other way round. A window that inverts those two roles will produce a flush that is correct and slow, and no case in this suite will say so.
>
> **What "beside the allocation" does not settle, recorded 2026-08-11 at the Window B review because two axes independently measured this line against the wording above and one of them expected the opposite.** The extension says the cursor is written late, in one statement, beside the allocation. **It does not order the cursor write against the allocation, and the order it does not fix is a real trade rather than a detail.** Both rows are held to commit, so the one taken first is held longest. Taking the **cursor** first spans the entry building, which this decision measured at 11.99 ms per 500 operations, on a row keyed by clientID, tenant and project and therefore contended only by that one installation racing its own resend. Taking it **after the append** costs the **version counter** one extra statement inside its own held span, on the row every client of the project shares, which is the contention this whole decision exists to shorten. **So the cursor is written first, and the reason is which row is contended rather than which reads better against the word "beside".**
>
> **Corrected 2026-08-11, hours after the paragraph was written, by the window that had to implement against it.** The first form said that writing the cursor after the append moves the **11.99 ms span** onto the counter. It does not, and the reason is one line of this decision's own earlier sharpening: `append_to_the_operation_log` already materialises its entries into a list **before** the allocation, so the entry building sits outside the counter's held span in **both** orders. The real trade is **one statement on the shared row against twelve milliseconds on a per-installation one**, which points the same way and is an order of magnitude smaller than stated. **The conclusion survived and the magnitude did not**, which is this project's named orchestrator failure mode: a measurement carried past the configuration it was taken in, this time by attaching a real number to the wrong span. The shape that is better than both and is not required here: build the entries before either statement, so no lock spans the serialisation at all. **The code implementing this needs an inline note**, because a reader checking it against the paragraph above will read the order as inverted and swap it. *(This sentence ended "and no case in the suite catches the swap" until 2026-08-18; the clause was struck at MAP-43's review, and the note below says why.)*
>
> **Sharpened 2026-08-18, at MAP-43's review, because that clause stopped being true the day the suite opened a second connection, and it was measured rather than noticed.** With `apply_the_flush` swapped to append-then-cursor, `test_a_flush_that_lost_the_race_does_not_lower_the_cursor_the_winner_already_raised` goes red, and it is the only red in the suite; the guard mutant of the same day (`GREATEST` collapsed to `EXCLUDED`) goes red at the cursor assertion. **But the swap is tripped over, not witnessed.** The harness holds the losing flush at the cursor statement, and under the swapped order that flush is already holding the project's version-counter row from its own append, so the winning flush blocks inside its allocation and never reaches the cursor: the case fails with the harness's own refusal, `TheRaceWasNotStaged: the winning flush never reached the cursor statement`, and not with an assertion about which row is contended. So the inline note the sentence above asks for stays in `services.py`, its own last sentence goes, and no case is claimed to witness the order. What a reader who swaps it now gets is a red build whose message names a staging invariant in a module they never touched, which is why the note is still needed rather than retired.
>
> **A flush left with nothing to apply takes no range at all** (moved here 2026-08-11 from the MAP-12 issue, where it was the only durable record of a permanent constraint on this path, and a ticket is execution state rather than contract). Once the dedup filter has emptied a batch there is nothing to order, and the allocating statement above **creates the counter row on first use**, so running it for width zero would mint a row for a project that has never been written to, out of a flush that changed nothing. The guard is on the caller and not inside the statement: **do not reach the allocation with an empty batch.**
...
>
> **So a log entry carries `project_id` and `project_version` as columns**, and the whole server half assembles at the read boundary once its remaining fields have owners.
> **Amended 2026-08-21 by ADR-0012 in two places.** A log entry carries a **third** column, `applied_at`,
> which was never among the four below and never undecided: T5.3 fixes it as the authoritative stamp and
> PRD M15 requires each chain entry to carry it, and the log was measured that day to hold no timestamp of
> any kind, so an applied operation's authoritative applied-at was not reconstructible from anything stored
> (MAP-53 lands the column). And **the list of four becomes three**, ADR-0012 decision 4 having given the
> per-feature version a home as a column on the projection row. The column is storage for the contract and never a second declaration of it; M8 stays the one definition.
>
> **The counter row is created on first use and does not reference the project table** (settled 2026-08-10 at the MAP-11 Window A review, which found the question already closed by evidence rather than open as the task spec had recorded it). **The principled half, measured:** nothing in `apps/api` creates a `Project` at all today. `mapsift/accounts/services.py` publishes account creation and no project creation, so the row only exists where a fixture makes one, and **eager creation has no place to live**. A hook invented for it would be a service written to satisfy a counter rather than a use case. *The principled half went stale on 2026-08-19, when MAP-47 published `create_workspace` and `create_project` in that very module, and the sentence is kept as the record of what was true when the decision was taken; the decision stands on the forcing half below, and a counter row created eagerly from the project-creation service would still be a service written to satisfy a counter, so nothing is reopened.* **The forcing half, also measured:** the flush suites that predate this task post batches addressing a project with no row in `accounts_project`, and an implementing window may not edit a test to make its code pass, so the allocation has to succeed for a project the table has never seen. That rules out a foreign key from the counter, which would also have had to be composite over the tenant under ADR-0005 section 5.
>
> **Amended 2026-09-17 by ADR-0014, in the same shape ADR-0012 amended this decision on 2026-08-21.** A log
> entry carries two further columns, `verdict` and `refusal_reason`, because the flush now decides each
> operation rather than the batch: a refused operation is written to the log as the client authored it,
> never dropped (PRD T5.2, M9, M15). **The list of three undecided server-half fields becomes two**, the
> verdict set having gained its second member and its second declarant there. Two consequences are the
> reader's rather than the writer's. A refused entry **consumes a per-project version** like any other, so
> the one-statement allocation of decision 2 is untouched and the ordering column stays non-null; and
> therefore **the resync read of this decision filters on the verdict**, since a resync stream is a stream
> of applied operations, which is MAP-22's clause to carry.
